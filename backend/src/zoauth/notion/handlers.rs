...
pub async fn get_authentication_url(
    ...
    let auth = state.services.notion.get_authorization_url(user_id).await;
    ...
    state
        .storage
        .redis
        .store_flow(user_id, "notion", &flow)
        .await?;

    Ok(axum::Json(auth.authorize_url))
}

pub async fn notion_callback(
    State(state): State<Arc<AppState>>,
    ...
    let auth = state.services.notion.exchange_code(params.code).await?;
    ...
    state
        .storage
        .redis
        .store_oauth_tokens(id, "notion", &token)
        .await?;
...
//     let auth = state.notion_client.get_authorization_url().await;
//
//     Ok(axum::Json(auth))
// }
